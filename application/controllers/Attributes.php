...
class Attributes extends Admin_Controller
{
	...
	public function fetchAttributeDataByCategoryID($id = null)
	{
		if ($id) {

			$result = array();
			$data = $this->model_attributes->getItemDataByCategory($id);

			if (is_array($data)) {
				foreach ($data as $key => $value) {
					// if (is_string($value)) { // added check for string
					// 	$value = json_decode($value, true); // convert string to array
			...
			// echo ($this->model_attributes->getItemDataByCategory(4));
		}
	...
	public function fetchAttributeData()
	{
		...
		foreach ($data as $key => $value) {
			...
			// button
			$buttons = '<button type="button" class="btn btn-default" id="material_edit" onclick="editFunc(' . $value['id'] . ')" data-toggle="modal" data-target="#editModal"><i class="fa fa-pencil"></i></button>
	...
	public function create()
	{
		...
		$this->form_validation->set_rules('attribute_price', 'Attribute Price', 'trim|required');
		$this->form_validation->set_rules('category', 'Category', 'trim|required');
		$this->form_validation->set_rules('attribute_quantity', 'Attribute Quantity', 'trim|required');
		...
		if ($this->form_validation->run() == TRUE) {
			$data = array(
				...
				'price' => $this->input->post('attribute_price'),
				'category_id' => $this->input->post('category'),
				'quantity' => $this->input->post('attribute_quantity'),
			);
	...
	public function update($id)
	{
		...
		if ($id) {
			...
			$this->form_validation->set_rules('edit_attribute_price', 'Attribute Price', 'trim|required');
			$this->form_validation->set_rules('edit_category', 'Attribute Category', 'trim|required');
			$this->form_validation->set_rules('edit_attribute_quantity', 'Attribute Quantity', 'trim|required');
			...
			if ($this->form_validation->run() == TRUE) {
				$data = array(
					...
					'price' => $this->input->post('edit_attribute_price'),
					'category_id' => $this->input->post('edit_category'),
					'quantity' => $this->input->post('edit_attribute_quantity')
				);
