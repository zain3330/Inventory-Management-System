...
class Orders extends Admin_Controller
{
	...
	public function create()
	{
		...
		if ($this->form_validation->run() == TRUE) {
			...
			// false case
			$company = $this->model_company->getCompanyData(2);
	...
	public function getProductValueById()
	{
		...
		if ($product_id) {
			$product_data = $this->model_products->getOrderProductData($product_id);
	...
	// 		if (!is_null($orders_data)) {
	// 			$result['order'] = $orders_data;
	// 			$orders_item = $this->model_orders->getOrdersItemData($orders_data['id']);

	// 			if (!is_null($orders_item)) {
	// 				foreach ($orders_item as $k => $v) {
	// 					$result['order_item'][] = $v;
	// 				}
	// 			}
	...
	public function update($id)
	{
		...
		if ($this->form_validation->run() == TRUE) {
			...
			$company = $this->model_company->getCompanyData(2);
			$this->data['company_data'] = $company;
			$this->data['is_vat_enabled'] = (!is_null($company) && $company['vat_charge_value'] > 0) ? true : false;
			$this->data['is_service_enabled'] = (!is_null($company) && $company['service_charge_value'] > 0) ? true : false;
	...
	public function printDiv($id)
	{
		...
		if ($id) {
			...
			$company_info = $this->model_company->getCompanyData(2);
			...
			foreach ($orders_items as $k => $v) {
				...
				$product_data = $this->model_products->getOrderProductData($v['product_id']);
